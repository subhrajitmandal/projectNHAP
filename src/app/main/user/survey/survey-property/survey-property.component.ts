...
export class SurveyPropertyComponent implements OnInit {
  ...
  constructor(
    ...
    this.surveyPropertyForm = this.fbuilder.group({
      ...
      district: [''],
      state: [''],
      oilCompanyName: ['', [Validators.required]],
      cityTowardsLeft: [
        '',
   ...
   */
  ngOnInit(): void {
    this.surveyDataService.surveyDataObs.pipe(take(1)).subscribe((data) => {
    ...
    this.surveyPropertyForm.valueChanges.subscribe(() => {
        ...
        this.surveyDataService.surveyDataObs
          .pipe(
            ...
            mergeMap((surveyData) => {
              ...
              if (surveyData.id) {
                return this.surveyService.saveDraft(
                  draftFormData,
                  surveyData.id
                );
              } else {
          ...
          .subscribe((draftDetails: Record<string, any>) => {
            this.surveyDataService.setStepFormData(
              this.surveyPropertyForm.controls,
              undefined,
              draftDetails.data.id
            );
          });
      });
    }
